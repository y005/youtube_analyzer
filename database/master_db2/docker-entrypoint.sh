...
setting_sharding_db() {
# 1. 레플리카 계정
    mysql -uroot -p'samho101' -e "CREATE USER 'samho101'@'%' IDENTIFIED BY '1234';"
    mysql -uroot -p'samho101' -e "GRANT REPLICATION SLAVE ON *.* TO 'samho101'@'%';"
    mysql -uroot -p'samho101' -e "flush privileges;"
# 2. 샤딩 계정
    mysql -uroot -p'samho101' -e "create user 'user'@'%' identified by '1234';"
    mysql -uroot -p'samho101' -e "grant all on *.* to 'user'@'%' with grant option;"
    mysql -uroot -p'samho101' -e "flush privileges;"
# 3. 데이터베이스 생성
    mysql -uroot -p'samho101' -e "create table if not exists project01.youtube_content(id int auto_increment primary key,title varchar(200) null,channel_name varchar(200) null,channel_id varchar(100) null,video_id varchar(100) null,view_count bigint null,like_count bigint null,dislike_count bigint null,published_time datetime null,comments text null,user_id varchar(100) null,percent double null,keywords varchar(200) null,subscribe_count bigint null) engine=innodb;"
}
